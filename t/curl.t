...
do
    echo "****** Run pachca_bot with template ${template_file} ******" >> bot.log 2>&1
    ./pachca_bot $(test -n "${template_file}" && echo "-d -t ${template_file}") >> bot.log 2>&1 &
    sleep 3
    for json_file in $json_files
    do
        result="$(curl -sw '\n%{http_code}' --data-binary "@$json_file" 127.0.0.1:9087/alert/${PACHCA_WEBHOOKID})"
