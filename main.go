...
type Config struct {
	PachcaWebhookUrl    string `yaml:"pachca_webhook_url"`
...
func main() {
	...
	router.POST("/alert/:webhookid", POST_Handling)
...
func AlertFormatStandard(alerts Alerts) string {
	...
	for i, a := range alerts.Alerts {
		...
		if a.GeneratorURL != "" {
			alertDetails[i] = fmt.Sprintf("[%s](%s)", alertDetails[i], a.GeneratorURL)
		}
	}
	return fmt.Sprintf(
		"[[%s:%d]](%s/#/alerts?receiver=%s)\ngrouped by: %s\nlabels: %s%s\n%s",
...
func POST_Handling(c *gin.Context) {
	...
	log.Printf("Webhook URL: %s/%s", cfg.PachcaWebhookUrl, webhookid)
	...
	for _, subString := range SplitString(msgtext, cfg.SplitMessageBytes) {
		...
		url := fmt.Sprintf("%s/%s", cfg.PachcaWebhookUrl, webhookid)
	
		request, err := http.NewRequest("POST", url, bytes.NewBuffer(jsonData))
		request.Header.Set("Content-Type", "application/json")
	
		client := &http.Client{}
		sendmsg, err := client.Do(request)
		if err == nil {
			c.String(http.StatusOK, "pachca msg sent.")
			fmt.Println("response Status:", sendmsg.Status)
		} else {
			log.Printf("Error sending message: %s", err)
			c.JSON(http.StatusServiceUnavailable, gin.H{
				"err":     fmt.Sprint(err),
				"message": sendmsg.Status,
